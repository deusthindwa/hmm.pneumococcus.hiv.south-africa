...
#forecasted total length of time spent in each trasient state
  ...
  phirst.es1 <- data.frame("iid"=c("HIV+ Child","HIV+ Adult"),"age"=c("Child","Adult"), "hiv"=c("HIV+","HIV+"),"status"=c("On ART","On ART"))
  phirst.es1$carry.est <- c(p.modele[1,2],p.modelg[1,2])
  phirst.es1$Lcarry.est <- c(p.modele[2,2],p.modelg[2,2])
  phirst.es1$Ucarry.est <- c(p.modele[3,2],p.modelg[3,2])
