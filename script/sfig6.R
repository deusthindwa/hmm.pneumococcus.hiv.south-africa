...
#recreate the dataset for misclassification investigation
phirst.mi <- phirst.fu
phirst.mi$state <- if_else(phirst.mi$state==1,1,if_else(phirst.mi$state==2,2,NULL))
phirst.mi$obst <- if_else(phirst.mi$dys==0,1,0)
...
p.miscla <- msm(state~dys,subject=ind_id,data=phirst.fu,
